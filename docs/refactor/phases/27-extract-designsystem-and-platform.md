...
2. `git diff -M --stat` shows every move as a rename with **100 % similarity**, except `AppIndex.swift`
   (which loses `IconCache`) and `IconCache.swift` (new).
...
~22 files moved, 2 docs updated, 1 file extracted. Content delta near zero.
...
- **`git diff -M --stat` is the whole review.** Anything under 100 % similarity that is not
  `AppIndex.swift` or `IconCache.swift` needs an explanation.
